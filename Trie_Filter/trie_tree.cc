...
void TrieTree::
killDescendants(TrieTree::Node *node, bool self) {
	TrieTree::Node *child = node->haschild();
	if(!child && self)
	{
		delete(node);
		node = NULL;
...
match_rule(const string &rule) {
	return checkFinalNode(match(root, rule));
}
 ...
 */
bool TrieTree::
deleteHelper(Node *pNode, const string &key, int level) {
	/*  for real delete */
	if( pNode ) 
	{
        // Base case
        ...
        else // Recursive case
        {
            ...
            if( deleteHelper(pNode->getchild(key[level]), key, level+1) )
            {
                ...
                pNode->delchild(key[level]);
 
                // recursively climb up, and delete eligible nodes
                return ( !pNode->isend && hasChildren(pNode) );
            }
        }
    }
...
deleteRule(string &key, bool real_delete) {
	...
	{
		TrieTree::Node *node = match(root, key);
		if(node && node->isend) 
		{
			node->isend = false;
			return true;
